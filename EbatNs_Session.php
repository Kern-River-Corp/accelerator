...
class EbatNs_Session {
   ...
   * @return boolean 
   */
  //function InitFromConfig($configFile, $site)
  function InitFromConfig($login)
  {
  	if(is_string($login)) $login = ebay_login::load($login);
  	//$login = new ebay_login($site);
  	$this->_keys['test'] = $login->keys('test');
  	$this->_keys['prod'] = $login->keys('prod');
  	$cfg = $login->settings;
    /*parse_ini_file($configFile, true)[$site];
    if ($cfg == false) {
    ...
    */
    /*if(isset($login->keys['token-prod'])){
    	$this->_props['RequestToken'] = $login->keys['token-prod'];
    }*/
    if (isset($cfg['site-id']))
        $this->setSiteId($cfg['site-id']);
    if (isset($login->user))
        $this->setRequestUser($login->user);
    if (isset($login->password))
        $this->setRequestPassword($login->password);
    /*if (isset($cfg['app-mode']))
        $this->setAppMode($cfg['app-mode']);*/
    if (isset($cfg['api-mode']))
        $this->setApiMode($cfg['api-mode']);
    ($cfg['sandbox']) ? $this->setAppMode(1) : $this->setAppMode(0);
    //($cfg['sandbox']) ? $this->_props['RequestToken'] = $login->keys['token-test'] : $this->_props['RequestToken'] = $login->keys['token-prod'];
    ($cfg['sandbox']) ? $this->setRequestToken($login->keys['token-test']) : $this->setRequestToken($login->keys['token-prod']);
   ...
   * @return void 
   */
  //function EbatNs_Session($configFile = null, $site = null)
  function EbatNs_Session($login)
  {
    // call to initialisation
    // (be sure to call this always on the actual class and prevent any overwriting)
    EbatNs_\core\session::_init();
    $this->_props['RequestToken'] = EBAY_NOTHING;
    if (isset($configFile)) {
      $this->InitFromConfig($configFile, $site);
    }
    else $this->InitFromConfig($login);
  }
  /**
